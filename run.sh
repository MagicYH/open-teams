...
start() {
    ...
    echo "正在后台启动后端服务 (FastAPI)..."
    cd "$BACKEND_DIR"
    nohup "$PYTHON_BIN" -m uvicorn app.main:app --host 127.0.0.1 --port 8000 > "$BACKEND_LOG" 2>&1 &
    echo $! > "$PROJECT_ROOT/backend.pid"
    echo "后端已启动，PID: $(cat "$PROJECT_ROOT/backend.pid")，日志: $BACKEND_LOG"

    # 启动前端
    echo "正在后台启动前端服务 (Vite)..."
    cd "$FRONTEND_DIR"
...
stop() {
    ...
    if [ -f "$PROJECT_ROOT/backend.pid" ]; then
        ...
        # 尝试通过端口查找并杀掉
        echo "未找到 backend.pid，正在尝试通过端口 8000 停止..."
    ...
    if [ -f "$PROJECT_ROOT/frontend.pid" ]; then
        ...
        echo "未找到 frontend.pid，正在尝试通过端口 5173 停止..."
        pkill -f "vite"
    fi
